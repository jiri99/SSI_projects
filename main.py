...
state_data["s"][0,:,0] = [5,10]
state_data["s"][1,:,0] = [10,10]
state_data["s"][2,:,0] = [15,10]
state_data["s"][3,:,0] = [20,10]
...
for t in range(0,len(time_discrete)-1):
    ...
    for ped_id_i in range(0, number_of_pedestrians):
        ...
        state_data["F_0"][ped_id_i,:,t] = F0/np.linalg.norm(F0)
    ...
    # Force projection
    for ped_id_i in range(0, number_of_pedestrians):
        # beta = math.atan(state_data["F"][ped_id_i,1,t]/state_data["F"][ped_id_i,0,t])
        # delta = math.atan(state_data["F_e"][ped_id_i,1,t]/state_data["F_e"][ped_id_i,0,t])
        # u0_i = np.dot(R(beta + state_data["q"][ped_id_i,0,t] - math.pi/2), state_data["F_e"][ped_id_i,:,t])
        # uf_i = np.dot(R(delta + state_data["q"][ped_id_i,0,t]), state_data["F"][ped_id_i,:,t])
        
        u0_i, uf_i = force_input(state_data["F_0"][ped_id_i,1,t], state_data["F_e"][ped_id_i,:,t], R(state_data["q"][ped_id_i,0,t])[:,0], R(state_data["q"][ped_id_i,0,t])[:,1], state_data["vB"][ped_id_i,1,t])
    
        state_data["u"][ped_id_i,:,t] = np.transpose([np.linalg.norm(uf_i), np.linalg.norm(u0_i)])
    ...
    # ODE solver
    for ped_id_i in range(0, number_of_pedestrians):
        uB = [state_data["uf"][ped_id_i,0,t],state_data["u0"][ped_id_i,0,t]]
        vB_dot = uB/m_pedestrian[ped_id_i]
        state_data["vB"][ped_id_i,:,t+1] = state_data["vB"][ped_id_i,:,t] + tau * vB_dot
        
        # state_data["v"][ped_id_i,:,t+1] = state_data["v"][ped_id_i,:,t] + tau * F0
        ...
        q_dot = np.dot(A, state_data["q"][ped_id_i,:,t]) + tau*np.dot(b(1/2*m_pedestrian[ped_id_i]*r_pedestrian[ped_id_i]), state_data["utheta"][ped_id_i,:,t])
        
        state_data["q"][ped_id_i,:,t+1] = state_data["q"][ped_id_i,:,t] + tau * q_dot
        
        # v_new = v + h * F(x, v, t);
        
        # F1 = lambda theta: np.dot(R(theta), state_data["v"][ped_id_i,:,t])
        
        
        
        # t_eval = [time_discrete[t]]
        # t_span = np.linspace(time_discrete[t-1], time_discrete[t], 101)
        
        # # Solve v
        # v0 = state_data["v"][ped_id_i,:,t]
        # sol = odeint(F0, v0, t_span)
        # state_data["v"][ped_id_i,:,t+1] = sol[len(sol)-1,:]
        
        # # Solve s
        
        # s0 = state_data["s"][ped_id_i,:,t]
        # sol = odeint(F1, s0, t_span)
        # state_data["v"][ped_id_i,:,t+1] = sol[len(sol)-1,:]
